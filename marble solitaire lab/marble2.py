...
def solve(puzzle):
    parseMe=[(puzzle, 0)]
    dictSeen={''.join(map(str,puzzle)):{0}} #string of puzzle:(parent,move to get there)
    while True:
        pzl, lvl = parseMe.pop(0)
        for nbr in neighbors(pzl[0]):
            nbrPzl= ''.join(map(str,nbr[0])) #str of puzzle
            newLvl = lvl + 1
            if nbrPzl not in dictSeen:
                parseMe.append((nbr[0], newLvl)) #list puzzle,move
                dictSeen[nbrPzl]=(''.join(map(str,pzl[0])), newLvl)#key = string of puzzle, value = string of parent, move


games = solve(startPuzzle)
distributions = {}

print(games)

#print(games['0000000000001000000000000'])
