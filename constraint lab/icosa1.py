...
def canAdd(choice, indepSet):
    for index in indepSet:
        for constrainedValue in constraints[index]:
            if choice == constrainedValue:
                return False
    return True
...
def solve(indepSet, choices):
    ...
    for choice in choices:
        if canAdd(choice, indepSet):
            newIndepSet = {index for index in indepSet}
            newIndepSet.add(choice)
            newChoices = {chc for chc in choices if chc != choice}
